...
chogan_render_buffer(Application_Links *app, View_ID view_id, Face_ID face_id,
    ...
    if (use_paren_helper && buffer != search_buffer){
    ...
    b32 highlight_range = def_get_config_b32(vars_save_string_lit("highlight_range"));
    if (highlight_line_at_cursor && is_active_view && !highlight_range){
